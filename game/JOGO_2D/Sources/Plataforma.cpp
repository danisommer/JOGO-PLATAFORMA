...
namespace Entidades
{
    namespace Obstaculos
    {
        ...
        void Plataforma::setFase(int f)
        {
            ...
            if (!grande) {
                sprite.setTexture(recursos->getTextura("Assets/Cenario/plataformaPequena.png"));
                sprite.setPosition(Vector2f(pos.x - 102.0f, pos.y - 75.0f));
                sprite.setScale(0.5f, 0.31f);
            }
            else
            {
                sprite.setTexture(recursos->getTextura(
                    "Assets/Cenario/plataformaGrande_" + std::to_string(fase) + ".png"));
